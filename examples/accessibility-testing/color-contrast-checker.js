...
async function scanPage(url) {
  ...
  const textElements = await page.evaluate(() => {
    const elements = [];
    const textTags = ['p', 'span', 'div', 'a', 'button', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'li', 'td', 'th', 'label'];
...
function printReport(report) {
  ...
  console.log(`  WCAG AA compliance: ${report.summary.complianceAA}% (${report.summary.passAA}/${report.summary.total})`);
  console.log(`  WCAG AAA compliance: ${report.summary.complianceAAA}% (${report.summary.passAAA}/${report.summary.total})`);
...
<body>
  ...
  <div class="failures">
    ...
    ${report.failures.aa.map((failure, index) => `
