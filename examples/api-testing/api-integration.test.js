...
const createApp = () => {
  ...
  app.post('/api/users', async (req, res) => {
    ...
    try {
      ...
      const duplicateCheck = await client.query('SELECT id FROM users WHERE email = $1', [email]);
  ...
  app.post('/api/posts', async (req, res) => {
    ...
    try {
      ...
      const authorCheck = await client.query('SELECT id FROM users WHERE id = $1', [author_id]);
  ...
  app.get('/api/external/weather', async (req, res) => {
    try {
      ...
      const externalResponse = await fetch(`https://api.weather.com/v1/weather?city=${city}`);
...
describe('API Integration Tests', () => {
  ...
  describe('Database Integration', () => {
    ...
    describe('POST /api/users', () => {
      it('should create user in database with transaction', async () => {
        ...
        // Verify in database
        const dbResult = await query('SELECT * FROM users WHERE email = $1', [newUser.email]);
      ...
      it('should rollback transaction on duplicate email', async () => {
        ...
        const dbResult = await query('SELECT COUNT(*) as count FROM users WHERE email = $1', [
          duplicateUser.email,
        ]);
    ...
    describe('PATCH /api/users/:id', () => {
      it('should update user in database with transaction', async () => {
        ...
        const response = await request(app).patch('/api/users/2').send(updates).expect(200);
        ...
        // Verify in database
        const dbResult = await query('SELECT * FROM users WHERE id = $1', [2]);
    ...
    describe('DELETE /api/users/:id', () => {
      it('should delete user and cascade to posts', async () => {
        ...
        const postsBefore = await query('SELECT * FROM posts WHERE author_id = $1', [1]);
        ...
        const userResult = await query('SELECT * FROM users WHERE id = $1', [1]);
        expect(userResult.rows.length).toBe(0);

        // Verify posts are also deleted (cascade)
        const postsAfter = await query('SELECT * FROM posts WHERE author_id = $1', [1]);
      ...
      it('should rollback transaction if user not found', async () => {
        ...
        await request(app).delete('/api/users/9999').expect(404);
    ...
    describe('POST /api/posts', () => {
      it('should create post with foreign key constraint', async () => {
        ...
        // Verify in database
        const dbResult = await query('SELECT * FROM posts WHERE id = $1', [response.body.id]);
      ...
      it('should fail when author does not exist', async () => {
        ...
        const response = await request(app).post('/api/posts').send(invalidPost).expect(404);

        expect(response.body.message).toContain('Author with ID 9999 not found');

        // Verify post was not created
        const dbResult = await query('SELECT * FROM posts WHERE title = $1', [invalidPost.title]);
  ...
  describe('External API Integration', () => {
    describe('GET /api/external/weather', () => {
      it('should fetch data from external API and cache it', async () => {
        ...
        const cacheResult = await query(`SELECT * FROM external_api_cache WHERE endpoint = $1`, [
          'weather:London',
        ]);
      ...
      it('should require city parameter', async () => {
        const response = await request(app).get('/api/external/weather').expect(400);
  ...
  describe('Transaction Isolation', () => {
    it('should isolate concurrent transactions', async () => {
      ...
      try {
        ...
        await client1.query('UPDATE users SET name = $1 WHERE id = $2', ['Transaction 1', 1]);

        // Start transaction 2
        await client2.query('BEGIN');

        // Transaction 2 should see old value
        const result2 = await client2.query('SELECT name FROM users WHERE id = $1', [1]);
        ...
        const result2After = await client2.query('SELECT name FROM users WHERE id = $1', [1]);
  ...
  describe('Connection Pool Management', () => {
    it('should handle multiple concurrent requests', async () => {
      ...
      const dbResult = await query('SELECT COUNT(*) as count FROM users WHERE email LIKE $1', [
        'concurrent%',
      ]);
  ...
  describe('Data Consistency', () => {
    it('should maintain referential integrity', async () => {
      ...
      await request(app).post('/api/posts').send(invalidPost).expect(404);

      // Verify post was not created
      const result = await query('SELECT * FROM posts WHERE title = $1', ['Invalid']);
    ...
    it('should cascade delete from users to posts', async () => {
      ...
      await query(`INSERT INTO posts (title, content, author_id) VALUES ($1, $2, $3)`, [
        'Post 1',
        'Content 1',
        userId,
      ]);
      ...
      const postsResult = await query('SELECT * FROM posts WHERE author_id = $1', [userId]);
